...
import wardrodeInsideImg from "../../../assets/img/subscenes/chairRoom/wardrobeInside203x257.png";
...
const Wardrobe = () => {
  ...
  const [wardrobeState, setWardrobeState] = useState<number>(
    puzzleUnlocked.wardrobe
      ? wardrobeStates.unlockedClosed
      : wardrobeStates.locked,
  );
  ...
  const openWardrobe = () => {
    ...
    if (
      wardrobeState == wardrobeStates.unlockedClosed &&
      puzzleUnlocked.wardrobe
    ) {
  ...
  return (
    ...
    >
      <div
        className="wardrobe-door-left"
        style={{
          display: wardrobeState === wardrobeStates.open ? "block" : "none",
        }}
        onClick={closeWardrobe}
      ></div>
      <div
        className="wardrobe-door-right"
        style={{
          display: wardrobeState === wardrobeStates.open ? "block" : "none",
        }}
        onClick={closeWardrobe}
      ...
      {wardrobeState === wardrobeStates.open && (
        <div
          className="wardrobe-inside-container"
          onMouseEnter={(e) => useLight(e)}
          onClick={showText}
        >
          <div
            className="wardrobe-inside"
            style={{
              backgroundImage: `url(${wardrodeInsideImg})`,
              backgroundPosition: `-${201.6}px 0px`,
              maskImage: activeItem == "firefly" ? `url(${lightImg})` : "",
              WebkitMaskImage:
                activeItem == "firefly" ? `url(${lightImg})` : "",
              maskPosition:
                activeItem == "firefly" ? `${lightX}px ${lightY}px` : "",
              display:
                puzzleUnlocked.wardrobeInside === true ||
                (activeItem == "firefly" && lightX != null)
                  ? "block"
                  : "none",
              opacity: "1",
            }}
          ...
          <div className="wardrobe-dark-text" style={{ opacity: text ? 1 : 0 }}>
            Too dark...
          </div>
